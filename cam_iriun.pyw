...
class ChromaKeyApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Application Chroma Key - Version Iriun Webcam")
        ...
        self.camera_index = 0  # Par défaut, on commence par l'index 0
    ...
    def detect_cameras(self):
        """Détecte automatiquement les caméras disponibles (dont Iriun)"""
        available_cameras = []
        for i in range(10):  # Tester les 10 premiers index
        ...
        if available_cameras:
            # Mettre à jour le menu déroulant avec les caméras disponibles
            self.camera_combo['values'] = [f"Caméra {i}" for i in available_cameras]
            self.camera_combo.current(0)
            self.camera_index = available_cameras[0]
            self.status_bar.config(text=f"Caméras détectées: {available_cameras}. Sélectionnez celle correspondant à Iriun")
        else:
    ...
    def setup_ui(self):
        ...
        # Titre
        ttk.Label(controls_frame, text="🎬 Contrôles Chroma Key Pro - Iriun", style='Title.TLabel').pack(pady=10)
        ...
        # Status bar
        self.status_bar = ttk.Label(self.root, text="Prêt - Sélectionnez votre caméra Iriun", relief=tk.SUNKEN)
        self.status_bar.pack(side=tk.BOTTOM, fill=tk.X)
        
    def create_camera_section(self, parent):
        """Section contrôle caméra avec sélection"""
        camera_frame = ttk.LabelFrame(parent, text="📷 Caméra Iriun", padding=10)
        ...
        self.camera_combo = ttk.Combobox(select_frame, state="readonly", width=15)
        self.camera_combo.pack(side=tk.LEFT, padx=5)
        self.camera_combo.bind('<<ComboboxSelected>>', self.on_camera_selected)
        
        ttk.Button(select_frame, text="🔄 Rafraîchir", command=self.detect_cameras).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(res_frame, text="Résolution:").pack(side=tk.LEFT)
        self.resolution_combo = ttk.Combobox(res_frame, values=["640x480", "1280x720", "1920x1080"], 
                                             state="readonly", width=10)
        self.resolution_combo.set("1280x720")
        self.resolution_combo.pack(side=tk.LEFT, padx=5)
        
    def on_camera_selected(self, event=None):
        """Met à jour l'index de la caméra sélectionnée"""
        selection = self.camera_combo.get()
        if selection:
            # Extraire le numéro de l'index
            try:
    ...
    def start_camera(self):
        if not self.is_running:
            # Utiliser l'index de caméra sélectionné
            self.camera = cv2.VideoCapture(self.camera_index)
            
            # Configurer la résolution selon la sélection
            ...
            if self.camera.isOpened():
                ...
                self.status_bar.config(text=f"Caméra Iriun active - Index {self.camera_index}")
            else:
                messagebox.showerror("Erreur", f"Impossible d'ouvrir la caméra {self.camera_index}")
