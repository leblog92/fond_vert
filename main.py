...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Cache UI : images déjà chargées et redimensionnées à 600px
        # clé = set_id, valeur = (background_bgr, foreground_rgba)
        self._ui_cache = {}

        # zone_info pour le montage final (pleine résolution)
    ...
    def _get_ui_assets(self, set_id):
        """Retourne (background_bgr, foreground_rgba) depuis le cache UI,
        en chargeant les fichiers UI_fond*.jpg / UI_pp*.png si nécessaire."""
        if set_id not in self._ui_cache:
            ...
            if not fond_path.exists() or not pp_path.exists():
                logger.warning(f"Fichiers UI manquants pour set {set_id}: {fond_path}, {pp_path}")
                return None, None

            bg = cv2.imread(str(fond_path))          # déjà à 600px
            pp = cv2.imread(str(pp_path), cv2.IMREAD_UNCHANGED)

            if bg is None or pp is None:
                logger.error(f"Impossible de lire les fichiers UI du set {set_id}")
                return None, None

            # Convertir pp en RGBA
            if pp.shape[2] == 3:
                pp_rgba = cv2.cvtColor(pp, cv2.COLOR_BGR2BGRA)
            else:
                pp_rgba = pp.copy()
                pp_rgba[:, :, 0:3] = cv2.cvtColor(pp[:, :, 0:3], cv2.COLOR_BGR2RGB)

            self._ui_cache[set_id] = (bg, pp_rgba)
            logger.info(f"Cache UI set {set_id} chargé: fond={bg.shape}, pp={pp_rgba.shape}")
    ...
    def update_image(self, frame):
        """Affichage caméra avec détourage (panneau gauche)"""
        self.current_frame = frame.copy()
        try:
            person_rgba, _ = self.processor.extract_person(frame)
    ...
    def update_montage_preview(self):
        """Aperçu du montage en direct — utilise les fichiers UI (600px)"""
    ...
    def update_set_preview(self):
        """Aperçu statique du set (fond + pp) dans le panneau droit — utilise le cache UI"""
        try:
            ui_bg, ui_pp = self._get_ui_assets(self.current_set)
            if ui_bg is None:
            ...
            # Composition rapide fond + pp sans personne
            fond_small = ui_bg.copy()
            if ui_pp is not None and ui_pp.shape[2] == 4:
                # Redimensionner pp à la taille du fond UI si nécessaire
                if ui_pp.shape[:2] != ui_bg.shape[:2]:
                    ui_pp_r = cv2.resize(ui_pp, (ui_bg.shape[1], ui_bg.shape[0]))
                else:
                    ui_pp_r = ui_pp
                alpha = ui_pp_r[:, :, 3] / 255.0
                for c in range(3):
                    fond_small[:, :, c] = (alpha * ui_pp_r[:, :, c] +
                                           (1 - alpha) * fond_small[:, :, c]).astype(np.uint8)
    ...
    def take_photo(self):
        ...
        try:
            assets_path = Path(__file__).parent / "assets"
            set_config = Config.SETS[self.current_set]
            fond_path = assets_path / set_config.fond_file   # fond*.jpg pleine résolution
            pp_path   = assets_path / set_config.pp_file     # pp*.png pleine résolution

            if not fond_path.exists():
                QMessageBox.critical(self, "Erreur", f"Fichier de fond non trouvé: {fond_path}")
                return
            if not pp_path.exists():
                QMessageBox.critical(self, "Erreur", f"Fichier de premier plan non trouvé: {pp_path}")
                return
